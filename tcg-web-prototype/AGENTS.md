...
- **Path rule:** scripts resolve relative to `__dirname` (e.g., `recall_ominous_test.js`, `build-cards.js`). No hardcoded `C:/Users/Blayne/Documents/...` paths anywhere. (Consumed Wave 6 batch scripts deleted 2026-08-31 — content merged into build-cards.js.)  
...
- **Authored card text/flavor (v0.1048+):** `textPatch`/`flavorPatch` (in build-cards.js transformCards) write optional `text` (final rules text — renderer uses it verbatim; keyword spans/footnotes still scanned) + `flavor` (italic flavor line on the card). Fields declared in `schema_definitions.json`. Engine behavior is UNCHANGED (`abilities` remain canonical for rules). Display-only fixes in `shared/effects.js`: `STRING_EFFECTS` (destroy/bounce enchantment) + pluralization cleanup. **Authoring flow:** add entries to the maps → `node build-cards.js build` + `init` + `verify`. Waves 1-7 shipped. Remaining un-authored non-land: **verify on arrival** (`docs/AUTHORING_BRIEF_WAVE8.md`). Voice: `docs/CREATIVE_IDENTITY.md`.
- **Type rebalance (2026-08-31):** old `gemini_rebalance_prompt.md` deleted; new approach points Gemini at `gemini_rebalance_input.tsv` (real 380 roster) + `gemini_vocabulary.txt` (legal vocab) so it cannot hallucinate ids/names/values. Rules: never invent/rename/reuse an id not in the TSV; key output by real int id; boosts only (ZERO-NERF); new mechanics in a `newMechanics` block; typeCounts must sum 380; rarity floors per faction preserved (8 Rare + 4 Mythic; Legendary count held). Omen cards require faceDownCost+flipTrigger+flipCost+flipAbilities.
...
Full guide: `docs/CREATIVE_IDENTITY.md` (**TENTATIVE** — refinement waves allowed). Voice = terse, aphoristic, wry; metaphor over description; no real-world or existing-game vocabulary. Faction palettes: Crimson=fire/blood/receipts, Sunforged=forest/growth, Lantern=grave/ledger-of-dead, Gilded=sea+wealth (tide/gold/ledger), Zealot=faith/dawn/blade, Colorless=machine/records/rust. Flavor = 1 sentence, 4-12 words. Rules `text` = one ability per `\n` line, engine-exact terms. Authoring flow unchanged (edit patch maps → build+init+verify). Remaining un-authored cards: **verify on arrival** — see `docs/AUTHORING_BRIEF_WAVE8.md` (2 batches of ~48).
