...
# Reusable smoke test for the project router: verifies health, the 6 request
# shapes (chat/responses/messages x stream/non-stream), and status auth.
# Exit code 0 = all pass, 1 = a shape failed, 2 = no router listening.
$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$Base = "http://127.0.0.1:8000"
$Pass = @(); $Fail = @()

# Load ROUTER_SHARED_SECRET from the same env files the router reads.
$Secret = ""
...
# --- ensure a single router is listening (unless -NoStart) ---
...
if (-not $bound -and -not $NoStart) {
    Out "Router not up; starting via start-router.ps1..."
    powershell -ExecutionPolicy Bypass -File (Join-Path $Root "start-router.ps1") -Background | Out-Null
    $bound = (Get-NetTCPConnection -LocalPort 8000 -State Listen -ErrorAction SilentlyContinue).OwningProcess
}
if (-not $bound) { Out "ERROR: no router listening on :8000 (use -NoStart to skip autostart)"; exit 2 }
Out "Router listener pid: $bound"

# --- 1. health (open) ---
Check "health" { Invoke-RestMethod -Uri "$Base/v1/health" -TimeoutSec 10 | Out-Null }

# --- 2. status without token must 401; with token must 200 ---
...
# --- helpers (auth on every real request) ---
